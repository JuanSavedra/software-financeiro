...
class OrganizadorFinanceiro(ctk.CTk):
    ...
    def construir_comparativo(self):
        ...
        # Grid para dividir em duas colunas + Divisor central
        frame.grid_columnconfigure(0, weight=1)
        frame.grid_columnconfigure(1, weight=0) # Divisor
        ...
        filter_l = ctk.CTkFrame(self.frame_comp_l)
        filter_l.pack(pady=(10, 0), padx=20, fill="x") # pady de baixo zero para aproximar
        ...
        self.canvas_comp_l = ctk.CTkFrame(self.frame_comp_l, fg_color="transparent")
        self.canvas_comp_l.pack(expand=True, fill="both")
        ...
        filter_r = ctk.CTkFrame(self.frame_comp_r)
        filter_r.pack(pady=(10, 0), padx=20, fill="x") # pady de baixo zero para aproximar
        ...
        self.canvas_comp_r = ctk.CTkFrame(self.frame_comp_r, fg_color="transparent")
        self.canvas_comp_r.pack(expand=True, fill="both")
    ...
    def atualizar_comparativo(self, lado):
        ...
        if df.empty:
            ctk.CTkLabel(parent, text="Sem dados.", font=("Arial", 14)).pack(expand=True); return

        if mes_val != "Todos": df = df[df['Data'].str.split('/').str[1] == mes_val]
        if ano_val != "Todos": df = df[df['Data'].str.split('/').str[2] == ano_val]

        if df.empty:
            ctk.CTkLabel(parent, text="Sem dados no período.", font=("Arial", 12)).pack(expand=True); return
        ...
        # Legenda em COLUNA (um embaixo do outro)
        ...
        for l, c, v in zip(labels, cores, valores):
            ...
            it.pack(pady=2, anchor="w")
            ctk.CTkFrame(it, width=12, height=12, fg_color=c, corner_radius=2).pack(side="left", padx=5)
            ctk.CTkLabel(it, text=f"{l}: R$ {v:,.2f}", font=("Arial", 11, "bold")).pack(side="left")
